  ...
  <div>
    ...
    <div>
      <el-form :inline="true" :model="formData" ref="formData" label-width="100px" class="demo-dynamic">
        ...
        <el-form-item>
          <el-button v-if="$route.meta.permission=='myProduct:update'" type="primary" @click="submitForm('formData')">提交</el-button>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList(){
         ...
         * @returns {number}
         */
        function covetNumber (value) {
          if(value)
            return value/1;
        }
        this.$http.get(this.moudules.product + '/product/'+this.$route.params.productId).then(response => {
          util.cover(response.data.data,this.formData);
          response.data.data.productIndexVOList.forEach(obj=>{
            if(obj.dataType==='FIGURE'){
              obj.indexValue = covetNumber(obj.indexValue);
              obj.upperLimitValue = covetNumber(obj.upperLimitValue);
              obj.floorLimitValue = covetNumber(obj.floorLimitValue);
            }
          })
          this.formData.productIndexList = response.data.data.productIndexVOList;
        });
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$http.post(this.moudules.product+'/myProduct/update', this.formData).then(() => {
